...
def calculate_file_statistics(content):
    ...
    information_content = {symbol: -1 * (symbol_frequencies[symbol] * prob * math.log2(prob)) for symbol, prob in
                           probabilities.items()}

    total_info_bits = sum(information_content.values())
    total_info_bytes = total_info_bits / 8

    sorted_alphabet = sorted(probabilities.items(), key=lambda x: x[0])

    sorted_frequencies = sorted(probabilities.items(), key=lambda x: x[1], reverse=True)

    print(f"File content length: {file_length}")
    print("Table (by letters):")
    print("Symbol\tFrequency\tProbabilities\tInformation")
    print_table(information_content, probabilities, sorted_alphabet)
    print("\nTable (by frequency):")
    print("Symbol\tFrequency\tProbabilities\tInformation")
    print_table(information_content, probabilities, sorted_frequencies)
...
def analyze_file(path):
    ...
    # Вывод результатов
    print(f"Octets: {most_common_octets}")
    print(f"Octets (not ASCII): {non_printable_ascii_octets}")

    is_russian(octet_frequencies)


def print_table(information_content, probabilities, dictionary):
    for symbol, frequency in dictionary:
        prob = round(probabilities[symbol], 10)
        info = round(information_content[symbol], 10)
